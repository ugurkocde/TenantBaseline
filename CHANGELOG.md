...
### Fixed
- `New-TBMonitor` adds `-SkipQuotaCheck` switch to bypass the pre-flight quota check when callers manage quota externally (#7)
...
### Fixed
- `Get-TBSnapshot` now includes `$select` with `errorDetails` in API requests (#3)
- `New-TBSnapshot` adds `-SkipQuotaCheck` switch (#5)
